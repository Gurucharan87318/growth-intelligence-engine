    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <PageContainer>
        <div className="space-y-10 py-14 md:py-16">
          ...
          <MetricsGridSkeleton count={3} className="grid gap-5 md:grid-cols-3" />
          ...
          <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <Card className="border-slate-200 bg-white shadow-sm">
              <CardHeader className="space-y-3">
                ...
                <Skeleton className="h-4 w-5/6 max-w-lg bg-slate-100" />
              </CardHeader>
              <CardContent className="grid gap-3">
                {Array.from({ length: 4 }).map((_, index) => (
                  <Skeleton key={index} className="h-14 w-full bg-slate-100" />
                ))}
              </CardContent>
            </Card>
